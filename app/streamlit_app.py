...
# --- BEHAVIOUR DISTRIBUTION (Pie chart — from InfluxDB query) ---
st.subheader("Behaviour distribution (from InfluxDB)")

import matplotlib.pyplot as plt

# Build SQL with filters (same filters as in your main query)
sql_pie = f"""
SELECT LOWER(TRIM(label)) AS label, COUNT(*) AS count
FROM sheep_behavior_pred
WHERE time >= TIMESTAMP '{start_iso}'
  AND time <= TIMESTAMP '{end_iso}'
{sheep_clause}{type_clause}{behaviour_clause}
GROUP BY label
ORDER BY count DESC
"""
...
try:
    with InfluxDBClient3(host=URL, token=TOKEN, org=ORG, database=DB) as client:
        result_pie = client.query(sql_pie)

    # Convert to DataFrame
    if isinstance(result_pie, pd.DataFrame):
        df_pie = result_pie
    elif isinstance(result_pie, pa.Table):
        df_pie = result_pie.to_pandas()
    elif isinstance(result_pie, list) and result_pie and isinstance(result_pie[0], pa.Table):
        df_pie = pd.concat([t.to_pandas() for t in result_pie], ignore_index=True)
    else:
        df_pie = pd.DataFrame(result_pie)

    if df_pie.empty:
        st.info("No data returned for pie chart.")
    else:
        # Clean and prepare
        df_pie["label"] = df_pie["label"].astype(str).str.strip().str.lower()
        known = [
            "flehmen", "grazing", "head-butting", "lying",
            "mating", "running", "standing", "walking",
        ]
        df_pie = df_pie.set_index("label").reindex(known, fill_value=0).reset_index()

        labels = df_pie["label"].tolist()
        counts = df_pie["count"].tolist()
        total = sum(counts)
        ...
        st.write("### Behaviour counts (InfluxDB query)")
        st.write(df_pie)

except Exception as e:
    st.error("Failed to query InfluxDB for pie chart.")
    st.exception(e)
